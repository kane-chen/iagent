---
name: stock-ticker
description: 按公司名查股票代码（东方财富搜索接口）。覆盖 A 股（沪深京）/港股/美股，补齐市场归属、公司类型、财报类型、监管机构；支持按交易所优先级排序，用于财报下载前的 ticker 归一化。触发词：股票代码、ticker、上市市场、腾讯代码、BABA 归属
---

# Stock Ticker Skill

**公司名 → 股票代码 + 市场归属 + 财报类型信息**（前身：`io.invest.iagent.tools.StockInfoTool`）
...
**下游用途**：`build_filing_kb`、`analyze_financial_trends`、`companyFilingQaAgent` 都需要先拿到规范化的 ticker + 市场类型才能选择合适的下载器与财报解析规则。
...
- 该 skill 无第三方依赖，只用 Python 标准库；`workspace/skills/stock-ticker/scripts/requirements.txt` 未提供是刻意的
