...
**从 workspace/excels/*.xlsx 里读结构化财务数据**（替代 `io.invest.iagent.service.retrieve.FinancialMetricsQueryService` 空壳 Java 实现）
...
`companyFilingQaAgent` 拿到这两个脚本的 JSON 输出后，把数字塞进 `[fact]` 表格，再叠加 `retrieve_filing_kb`（[filing-stated cause]）+ web 搜索（[external]）综合分析。
...
`companyFilingQaAgent` 的 `[fact]` 数据源里应加入本 skill：
...
优先级：本 skill 是"结构化事实"的唯一入口，覆盖全部市场且延迟毫秒级。Java 侧原有的 `analyze_financial_trends` 工具（SEC XBRL 在线拉取）已下线；`workspace/excels/` 由 `futu-financial-report` / `segment-financial-report` 两个 skill 负责生成。

## Java 端状态

- **已删除**：整个 `io.invest.iagent.service.retrieve.*` 包（含 `FinancialMetricsQueryService` / `FinancialTrendAnalysisService` 等），以及 `FinancialTrendAnalysisTool`。所有相关 [fact] 查询能力都统一到本 skill。
- **保留**：Java 侧不再持有任何"财务指标查询" 逻辑；Excel 生成方在 skill 层（`futu-financial-report` / `segment-financial-report`）。
