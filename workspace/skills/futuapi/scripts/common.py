...
# 适配新版本 futu API 的导入方式
try:
    # 旧版本导入方式（QClaw 自带 Python）
    from futu import (
            OpenQuoteContext,
            OpenSecTradeContext,
            RET_OK,
    ...
    # 过滤器类在不同位置
    from futu.quote.quote_query import SimpleFilter, AccumulateFilter, FinancialFilter

try:
    from futu import TradeDateMarket
except ImportError:
    try:
        from futu.common.constant import TradeDateMarket
    except ImportError:
...
try:
    ...
    try:
        from futu.common.constant import SecurityFirm
    except ImportError:
        SecurityFirm = None
...
try:
    ...
    try:
        from futu.trade.open_trade_context import OpenCryptoTradeContext
    except ImportError:
...
try:
    ...
    try:
        from futu.common.constant import TimeInForce
    except ImportError:
        TimeInForce = None
