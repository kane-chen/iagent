...
python workspace/skills/financial-qa/scripts/qa.py \
...
| `--top-k` | | 返回片段数，默认 5 |
| `--backend` | | 覆盖 backend：milvus / ragflow |
| `--pretty` | | JSON 输出时缩进 |
...
```json
{
  ...
  "llm": { "baseUrl": "http://localhost:11434/v1", "apiKey": "local", "model": "qwen3:4b", "temperature": 0.2, "maxTokens": 2048 }
}
```
