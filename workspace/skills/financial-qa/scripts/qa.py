#!/usr/bin/env python3
"""CLI entry for financial-qa skill.
...
def do_answer(args, cfg):
    ...
    if backend == "ragflow":
        chunks = _ragflow.search(
            query=args.question, ticker=ticker, top_k=top_k * 2,
            form_type=args.form_type, fiscal_period=fiscal_period,
            from_year=from_year, to_year=to_year, keyword=args.keyword,
            similarity_threshold=sim_thresh, recall_multiplier=recall_mult,
            ragflow_cfg=cfg["ragflow"], embedding_cfg=cfg["embedding"],
    ...
    model = cfg["llm"].get("model", "qwen3:4b")
    elapsed_ms = 0
    try:
...
def main():
    p = argparse.ArgumentParser(description="Financial QA CLI")
