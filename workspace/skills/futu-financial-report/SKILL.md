...
| 中蓝 `#BDD7EE` | 加工指标 | 经营活动现金流量净额、FCF、CapEx、ROA、ROE |
...
- **现金流量表（cashflow）**：`--num` = **季度数**（v0.7.8+）。现金流量表也与利润表同频（HK/A股 累计季报，美股单季+年报），2026Q1 利润表 ↔ 2026Q1 现金流量表（1–3 月累计现金流），供 FCF/OCF 精准对齐。
...
7. **利润表加工指标 FCF 依赖现金流量表数据**：`FCF = (调整后) 经营活动现金净流入 − 经营性资本开支`，两项均来自现金流量表。脚本在生成利润表时会自动同步拉取当期现金流量表数据，无需额外操作；若底层现金流拉取失败会跳过 FCF/CapEx 加工指标，此时可让用户单独跑一次 `--type cashflow` 生成现金流量表 Excel 兜底核对。此外，利润表加工指标区域会额外展示 `经营活动现金流量净额`(OCF) 与 `资本开支`(CapEx) 两行，方便对照 `FCF = OCF - CapEx` 的推导。

8. **生成利润表时自动产出配套现金流量表 Excel**：与资产负债表的处理一致，如果 `workspace/excels/` 下没有 7 天内的同代码现金流量表 Excel，脚本会**复用刚拉取的现金流数据**同步生成一份现金流量表 Excel（不重复调用 API）；若已存在近期文件则直接复用不重复生成。生成失败仅告警，不影响利润表主输出。
