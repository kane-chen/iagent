...
A_SHARE_FIELDS = {
    ...
    'cashflow': [
        ...
        (3002, '折旧与摊销', 'cash_in'),                 # D&A 调整
        (3003, '财务费用', 'cash_in'),                   # D&A 调整（利息支出非现金项）
        (3004, '存货的减少', 'cash_in'),                 # 存货变动
...
US_FIELDS = {
    'income': [
        ...
        (8005, '营业费用',        'expense'),
        (8017, '营业利润',        'profit'),
        (8011, '折旧与摊销',      'expense'),        # D&A（部分公司在利润表列示）
        (8012, '财务费用',        'expense'),
        (8013, '利息费用',        'expense'),        # 利息支出净额
        (8034, '税前利润',        'profit'),
        (8035, '所得税',          'tax'),
        (8037, '净利润',         'profit'),
...
def get_row_category(row_name):
    ...
    category_markers = [
        ...
        ('折旧', 'cash_in'),
        ('摊销', 'cash_in'),
        ('存货', 'cash_out'),     # 存货变动归营运资本（浅红）
        ('应收', 'cash_in'),      # 应收减少→现金流入（浅绿）
        ('应付', 'cash_out'),     # 应付变动归营运资本（浅红）
        ('税', 'tax'),
        ('率', 'ratio'),  # 其他比率 - 黄色背景
        ('YoY', 'yoy'),
    ]
